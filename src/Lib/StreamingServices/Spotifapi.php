...
namespace Sevenedge\Lib\StreamingServices;

use Sevenedge\Lib\Utilities;
...
class Spotifapi
{
	...
	private static $templates = array(
		'landinguri' => 'https://accounts.spotify.com/be/login?continue=https:%2F%2Faccounts.spotify.com%2Fauthorize%2F%3Fclient_id%3D{client_id}%26redirect_uri%3D{redirect_uri}%26response_type%3Dcode%26scope%3D{scope}%26show_dialog%3Dfalse%26state%3D',
		'loginuri' => 'https://accounts.spotify.com/api/login',
	 ...
	 */
	public function __construct($clientId, $clientSecret, $redirect_uri, $scope = array(), $credentials, $logCallback = null)
	{
		...
		$this->cr = new CurlRequest();

		//filling out the templates
		$landinguri = str_replace("{client_id}", $this->clientId, self::$templates['landinguri']);
		$landinguri = str_replace("{scope}", urlencode(implode(" ", $this->scope)), $landinguri);
		self::$templates['landinguri'] = str_replace("{redirect_uri}", urlencode(urlencode($this->redirect_uri)), $landinguri);
		...
		$authenticationuri = str_replace("{scope}", urlencode(implode(" ", $this->scope)), $authenticationuri);
	...
	public static function generateAuthorizationURI($clientId, $scope = array(), $redirect_uri)
	{
		$landinguri = str_replace("{client_id}", $clientId, self::$templates['landinguri']);
		$landinguri = str_replace("{scope}", urlencode(implode(" ", $scope)), $landinguri);
		return str_replace("{redirect_uri}", urlencode(urlencode($redirect_uri)), $landinguri);
	...
	private function _getAuthenticationDetails()
	{
		if (isset($this->credentials['access_token'])) {
			if ($this->credentials['valid_until'] > time()) {
				call_user_func($this->logCallback, E_USER_NOTICE, "using the cached access token");
				...
				if ($err === 0) {
					...
					call_user_func($this->logCallback, E_USER_NOTICE, "was able to get a new access token with the refresh token");
					return $this->credentials;
				}
				$this->cr->clean();
			}
		}
		call_user_func($this->logCallback, E_USER_WARNING, "current token expired and unable to get a new one with the refresh token. trying to log in");
	// step one: we need that cookie & csrf to bypass the cross site request forgery checks. we be stealthy!
		...
		if ($err === 0) {
			...
			if ($matchcount) {
				...
				$matchcount = preg_match('/csrf_token=([^;]+);?.*$/mi', $matches[1], $matches);
				...
				if ($matchcount) {
					...
					$key = $this->cr->addRequest(self::$templates['loginuri'],
						...
						array(
							CURLOPT_FOLLOWLOCATION => false,
							CURLOPT_REFERER => self::$templates['authenticationuri'],
		...
		call_user_func($this->logCallback, E_USER_ERROR, "Failed to get an access token in any possible way");
	...
	public function createList($name, $public = true)
	{
		...
		call_user_func($this->logCallback, E_USER_ERROR, "Failed to create a new list called '$name'. response was " . print_r($this->cr->getResponse(), 1));
		$this->cr->clean();
		return null;
	}
	...
	public function getPlaylistTracks($listId)
	{
		...
		call_user_func($this->logCallback, E_USER_ERROR, "Failed to get the playlist $listId tracks. response was " . print_r($this->cr->getResponse(), 1));
		$this->cr->clean();
		return null;
	}
