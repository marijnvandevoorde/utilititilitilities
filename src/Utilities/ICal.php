...
class ICal
{
	...
	private /** @type {int} */ $_todo_count = 0;

	/* How many events are in this ical? */
	private /** @type {int} */ $_event_count = 0;

	/* How many freebusy are in this ical? */
	private /** @type {int} */ $_freebusy_count = 0;

	/* The parsed calendar */
	private /** @type {Array} */ $_cal;

	/* Which keyword has been added to cal at last? */
	private /** @type {string} */ $_last_keyword;

	/* The value in years to use for indefinite, recurring events */
	private /** @type {int} */ $_default_span = 2;
	...
	public static function createFromURI($uri) {
		$ical = null;
		$cr = new CurlRequest();
		$key = $cr->addRequest($uri);
		$err = $cr->execute();
		if ($err === 0) {
			$res = $cr->getResponse($key);
			$cr->clean();
			if ($res['http_code'] > 399) {
				throw new UnauthorizedException("Fetching ical failed: " . print_r($res, 1));
			}
			$ical = new ICal();
			return $ical->_import($res['response']);
		}
	...
	public static function createFromFile($path) {
		...
		$ical = new ICal();
		return $ical->_import(file_get_contents($path));
	}
		...
		if (stristr($lines[0], 'BEGIN:VCALENDAR') === false) {
			...
			foreach ($lines as $idx => $line) {
				...
				if (!preg_match('#^([^:]+):(.*)$#', $line, $matches) && $currentKey) {
					if ($currentKey) {
						$currentKey['value'] .= substr($line, 1);
					}
					continue;
				}
				if ($currentKey) {

					switch ($currentKey['key'] . ':' . $currentKey['value']) {
						...
						case 'END:STANDARD':
							...
							break;
						default:
							$this->addCalendarComponentWithKeyAndValue($type, $currentKey['key'], trim($currentKey['value'], " \t\n\r\0\x0B\\n"));
							break;
					}
				}
				$currentKey = [
	...
	public function addCalendarComponentWithKeyAndValue($component, $keyword, $value)
	{
		...
		if ($keyword == false) {
			...
			switch ($component) {
				case 'VEVENT':
					$value = $this->_cal[$component][$this->_event_count - 1][$keyword] . $value;
					break;
				case 'VTODO':
					$value = $this->_cal[$component][$this->_todo_count - 1][$keyword] . $value;
					break;
				case 'VFREEBUSY':
					$value = $this->_cal[$component][$this->_freebusy_count - 1][$keyword] . $value;
					break;
			}
		}
		...
		switch ($component) {
			case 'VTODO':
				$this->_cal[$component][$this->_todo_count - 1][$keyword] = $value;
				break;
			case 'VEVENT':
				$this->_cal[$component][$this->_event_count - 1][$keyword] = $value;
				if (!isset($this->_cal[$component][$this->_event_count - 1][$keyword . '_array'])) {
					$this->_cal[$component][$this->_event_count - 1][$keyword . '_array'] = array();
				}
				$this->_cal[$component][$this->_event_count - 1][$keyword . '_array'][] = $value;
				break;
			case 'VFREEBUSY':
				$this->_cal[$component][$this->_freebusy_count - 1][$keyword] = $value;
				break;
			default:
				$this->_cal[$component][$keyword] = $value;
				break;
		}
	...
	public function process_recurrences()
	{
		...
		foreach ($array['VEVENT'] as $anEvent) {
			if (isset($anEvent['RRULE']) && $anEvent['RRULE'] != '') {
				...
				$until_default->modify($this->_default_span . ' year');
	...
	{
		return $this->_cal['VCALENDAR']['X-WR-CALNAME'];
	}
